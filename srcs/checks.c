...
/*   Updated: 2024/03/22 16:33:53 by kabasolo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	checks(t_game *data)
{
	...
	if (find(copy, 'E') != 0)
		return (freemap(copy),
			error("The player must be able to reach the exit."));
	if (find(copy, 'C') != 0)
		return (freemap(copy),
			error("All the coins must be accesible for the player"));
	if (data->col * PIX > 2500 || data->lines * PIX > 1400)
		return (freemap(copy), error("Map is too big for this screen."));
