...
/*   Updated: 2024/03/21 16:12:59 by kabasolo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	**get_map(int fd)
{
	...
	while (byte != 0)
	{
		byte = read(fd, str, 1);
		if (byte == -1)
			return (free(map), error("Lecture failure."), NULL);
		str[byte] = '\0';
		if (str[0] == '\n' && (!map[0] || map[ft_strlen(map) - 1] == '\n'))
			return (free(map), error("Non-valid map format."), NULL);
		temp = map;
		map = ft_strjoin(temp, str);
		free(temp);
	}
